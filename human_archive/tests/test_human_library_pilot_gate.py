# -*- coding: utf-8 -*-
"""
test_human_library_pilot_gate.py
Automated test suite verifying Gate 3 (Pacing), Gate 4 (Audio Contract),
Gate 5 (Asset Contract), and Gate 6 (Render Motion Binding) for Pilot-5 (20.6s).
"""
...
def pilot_bundle() -> dict:
    assert PILOT_BUNDLE_FILE.exists(), f"Pilot bundle missing: {PILOT_BUNDLE_FILE}"
    with open(PILOT_BUNDLE_FILE, "r", encoding="utf-8") as f:
        return json.load(f)


def test_pilot_bundle_exists_and_conforms_to_schema(pilot_bundle):
    ...
    assert pilot_bundle["target_duration_sec"] == 20.6
    assert len(pilot_bundle["shots"]) == 5


def test_pilot_gate3_pacing(pilot_bundle):
    res = verify_gate3_pacing(pilot_bundle)
    assert res["status"] == "PASS"
    assert res["total_duration_sec"] == 20.6
...
def test_pilot_gate4_audio_contract_and_fail_closed(pilot_bundle):
...
def test_pilot_gate5_asset_contract_and_prompt_safety(pilot_bundle):
...
def test_pilot_gate6_render_motion_binding(pilot_bundle):
    res = verify_gate6_render_motion_binding(pilot_bundle)
    assert res["status"] == "PASS"
    assert res["total_bindings"] == 5

    expected_canonical = ["push_in", "pan_right", "pull_out", "push_in", "pan_left"]
...
def test_pilot_full_verification_report():
