# -*- coding: utf-8 -*-
"""run_human_library_full_production.py

Full Production Pipeline for Human Library Replica Rank 1 ("같은 인간인데 왜 이렇게까지 다를까"):
- All 40 Shots (SHOT_001 to SHOT_040)
- Step 1: SuperTonic3 M2 Narration Audio Synthesis + 0.40s Roomtone Padding (48kHz Stereo)
- Step 2: Google Flow CDP 35mm Documentary Stills Generation + 1080p Normalization + DOM GC
- Step 3: Smooth Subpixel Motion Rendering (Push-in, Pull-out, Pan, Tilt, Tri-phasic)
...
- Step 6: Final Multiplex into rank1_master_documentary.mp4
- Step 7: Postflight Gate 8 Decoded Frame Analysis
- Step 8: Physical Release Manifest Generation (SHA-256 Chain)
"""
...
from lib.asset_contract import evaluate_frame_visibility_from_file, evaluate_frame_visibility_gate
...
BUNDLE_PATH = EP_DIR / "metadata" / "normalized_replica_bundle.json"
...
# =========================================================================
# STEP 1: Audio Narration Synthesis & Frame-Aligned Padding
# =========================================================================

def synthesize_all_audio(shots: List[Dict[str, Any]]) -> Tuple[Path, float]:
    print("\n=======================================================")
    ...
    sample_rate = 48000
    fps = 25
    samples_per_frame = sample_rate // fps  # 1920
    ...
    for idx, s in enumerate(shots, 1):
        ...
        if not raw_wav.exists() or raw_wav.stat().st_size < 5000:
            print(f"[{idx:02d}/{len(shots)}] Synthesizing {shot_id}: \"{text[:32]}...\"")
            ...
            print(f"[{idx:02d}/{len(shots)}] Using cached raw audio for {shot_id}")

        # 2. Convert to 48kHz Stereo 16-bit PCM if needed
    ...
    # Update timeline cumulatively across all 40 shots
    cur_t = 0.0
    for s in shots:
    ...
    master_voice_wav = EP_DIR / "audio" / "rank1_master_voice_48k.wav"
    ...
    return master_voice_wav, master_dur


# =========================================================================
# STEP 2: Google Flow CDP Image Generation (All 40 Shots)
# =========================================================================

async def generate_all_flow_images(shots: List[Dict[str, Any]]) -> List[Path]:
    print("\n=======================================================")
    print(f"🎨 [STEP 2] Google Flow CDP Batch Image Generation ({len(shots)} Shots)")
    print("=======================================================")
    ...
    async with async_playwright() as p:
        ...
        for idx, s in enumerate(shots, 1):
            ...
            if out_img.exists() and out_img.stat().st_size > 50000:
                print(f"[{idx:02d}/{len(shots)}] Image cached for {shot_id} ({out_img.stat().st_size:,} bytes). Skipping.")
                if not appr_img.exists():
                    shutil.copy2(out_img, appr_img)
                result_images.append(out_img)
                continue

            print(f"\n[{idx:02d}/{len(shots)}] Submitting prompt for {shot_id}...")
            ...
            print(f"   Gate 8 Evaluation: passed={eval_res['passed']} (edge={eval_res['edge_density']}, coverage={eval_res['non_bg_coverage']})")
...
# =========================================================================
# STEP 3: Subpixel Motion Video Clip Rendering (All 40 Shots)
# =========================================================================

def render_all_motion_clips(shots: List[Dict[str, Any]], images: List[Path]) -> Path:
    print("\n=======================================================")
    ...
    # Map motion profile to engine motion string
    def resolve_motion(s: Dict[str, Any]) -> str:
        mp = s.get("motion_profile", {})
        fam = mp.get("motion_family", "push_in")
        axis = mp.get("axis", "")

        if fam in ("tri_phasic", "tri_phasic_ken_burns", "long_take_tri_phasic"):
            return "tri_phasic"
        if fam == "pan":
            return "pan_left" if "left" in axis else "pan_right"
        if fam == "tilt":
            return "tilt_up" if "up" in axis else "tilt_down"
        if fam == "pull_out":
            return "pull_out"
        return "push_in"
    ...
    for idx, (s, img_path) in enumerate(zip(shots, images), 1):
        ...
        motion = resolve_motion(s)
        out_clip = CLIPS_DIR / f"{shot_id}.mp4"

        # Check if clip exists and has exact duration
        if out_clip.exists() and out_clip.stat().st_size > 50000:
            real_dur = get_ffprobe_duration(out_clip)
            if abs(real_dur - dur) <= 0.05:
                print(f"[{idx:02d}/{len(shots)}] Cached clip {out_clip.name} ({real_dur:.3f}s). Skipping.")
                clip_paths.append(out_clip)
                continue

        print(f"[{idx:02d}/{len(shots)}] Rendering {shot_id}.mp4: motion='{motion}', duration={dur:.2f}s, 1920x1080 @ 25fps...")
    ...
    concat_video = EP_DIR / "video" / "rank1_concat_video.mp4"
...
# =========================================================================
# STEP 4: ASS Subtitles Compilation (All 40 Shots)
# =========================================================================

def compile_all_subtitles(shots: List[Dict[str, Any]]) -> Path:
    print("\n=======================================================")
    print("📝 [STEP 4] 52pt Pretendard ASS Subtitle Compilation")
    print("=======================================================")

    SUBTITLES_DIR.mkdir(parents=True, exist_ok=True)
    ass_path = SUBTITLES_DIR / "rank1_master.ass"

    engine = SemanticSubtitleEngine(font_size=52, max_line_chars=36, max_clause_chars=70)
    engine.compile_ass_subtitles(shots=shots, output_path=ass_path, title="같은 인간인데 왜 이렇게까지 다를까 (Human Library Replica)")
...
# =========================================================================
# STEP 5 & 6: Pre-Mux Parity & Master Multiplexing
# =========================================================================

def mux_full_master(concat_video: Path, master_audio: Path, ass_path: Path) -> Path:
    print("\n=======================================================")
    print("🎛️ [STEP 5 & 6] Pre-Mux Parity & Master Multiplexing")
    print("=======================================================")

    CANDIDATE_DIR.mkdir(parents=True, exist_ok=True)
    master_mp4 = EP_DIR / "rank1_master_documentary.mp4"
    ...
    # Mux with subtitle burning via sub filter for broadcast visual fidelity
...
def verify_gate8_full_master(master_mp4: Path, shots: List[Dict[str, Any]]) -> Dict[str, Any]:
    print("\n=======================================================")
    ...
    cap.release()
    passed_count = sum(1 for b in boundary_evaluations if b["evaluation"]["passed"])
    all_passed = (passed_count == len(boundary_evaluations))
    print(f"Gate 8 Result: {passed_count}/{len(boundary_evaluations)} boundaries passed ({'ALL PASSED ✅' if all_passed else 'SOME FAILED ❌'})")
    return {
        "passed": all_passed,
        "width": width,
        "height": height,
        ...
        "total_boundaries": len(boundary_evaluations),
...
def generate_full_release_manifest(
    master_mp4: Path,
    master_audio: Path,
    concat_video: Path,
    images: List[Path],
    ...
    manifest_path = METADATA_DIR / "rank1_release_manifest.json"

    manifest_data = {
        "schema_version": "1.0.0",
        "release_id": f"human_library_replica_rank1_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}",
        "created_at": datetime.now(timezone.utc).isoformat(),
        "episode_id": "human_library_rank1_race_adaptation",
        "title": "같은 인간인데 왜 이렇게까지 다를까 (Human Library Replica Full Master)",
        "total_shots": len(shots),
        "target_duration_sec": 972.11,
        ...
        "gate8_verification": {
            "passed": gate8_report["passed"],
            "passed_count": gate8_report["passed_count"],
            "total_boundaries": gate8_report["total_boundaries"],
        },
        "shots": []
    }
    ...
    print(f"✅ Full Release Manifest Saved: {manifest_path}")
...
def main():
    print("=================================================================")
    print("🚀 LAUNCHING FULL 40-SHOT REPLICA GENERATION & RENDERING")
    print("   Episode: 같은 인간인데 왜 이렇게까지 다를까")
    print("=================================================================")
    ...
    # 1. Synthesize audio for all 40 shots
    master_audio, master_dur = synthesize_all_audio(shots)

    # Save updated active timeline bundle
    active_bundle_path = METADATA_DIR / "rank1_active_timeline_bundle.json"
    active_bundle_path.write_text(json.dumps({"shots": shots, "total_duration_sec": master_dur}, indent=2, ensure_ascii=False), encoding="utf-8")
    print(f"Active timeline bundle saved: {active_bundle_path.name}")

    # 2. Generate Flow images for all 40 shots
    images = asyncio.run(generate_all_flow_images(shots))

    # 3. Render motion clips for all 40 shots
    concat_video = render_all_motion_clips(shots, images)

    # 4. Compile subtitles for all 40 shots
    ass_path = compile_all_subtitles(shots)

    # 5 & 6. Mux master video
    master_mp4 = mux_full_master(concat_video, master_audio, ass_path)

    # 7. Gate 8 verification
    gate8_report = verify_gate8_full_master(master_mp4, shots)

    # 8. Release manifest
    manifest_path = generate_full_release_manifest(master_mp4, master_audio, concat_video, images, shots, gate8_report)

    print("\n=================================================================")
    print("🎉 FULL 40-SHOT REPLICA GENERATION & PHYSICAL VERIFICATION COMPLETE!")
