...
def build_prompt_context(contract: dict, inventory: dict, snapshots: dict, policy: dict) -> str:
    template_path = _SCRIPTS_DIR.parent / "templates" / "seonbi_script_prompt.j2"
...
def generate_script_candidate(
    ...
    prompt = build_prompt_context(contract, inventory, snapshots, policy)
    schema_path = _SCRIPTS_DIR.parent / "schemas" / "verified_script_v2.schema.json"
