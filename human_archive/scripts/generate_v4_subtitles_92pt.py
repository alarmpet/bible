...
def split_sentence_into_clauses(text: str, max_chars: int = 32) -> list[str]:
    ...
    for pos in comma_matches:
        diff = abs(pos - mid)
        if diff < min_diff and pos <= len(text) - 6 and pos >= 6:
            min_diff = diff
            best_split = pos
...
def main():
    ...
    print(f"Loaded {len(shots)} shots for 92pt subtitle compilation.")

    # 92pt Bold: 80% of 116pt, Outline 6.0, Shadow 3.2, MarginV 85, MarginL 140, MarginR 140
    header = """[Script Info]
ScriptType: v4.00+
PlayResX: 1920
PlayResY: 1080
...
Style: DocuNarrator_v4,Pretendard,92,&H00FFFFFF,&H000000FF,&H000C0C12,&H90000000,-1,0,0,0,100,100,0,0,1,6.0,3.2,2,140,140,85,1
...
"""
    ...
    for s in shots:
        ...
        # HARD INVARIANT: Subtitle for this shot MUST NOT exceed scene_end - 0.15s
        max_shot_sub_end = scene_end - 0.15
        available_sub_dur = max(1.0, max_shot_sub_end - speech_start)
        effective_dur = min(speech_dur, available_sub_dur)
        ...
        for idx, sent in enumerate(raw_sents):
            ...
            sent_dur = max(1.4, round(effective_dur * ratio, 3))
            
            # Subdivide into clauses (max 32 chars for 92pt font with longer lines)
            ...
            for c_idx, clause in enumerate(clauses):
                ...
                c_dur = max(1.0, round(sent_dur * c_ratio, 3))
                
                # STRICT NON-OVERLAPPING:
                sub_start = round(clause_t, 2)
                sub_end = min(max_shot_sub_end, round(clause_t + c_dur - 0.05, 2))
                if sub_end <= sub_start:
                    sub_end = min(max_shot_sub_end, sub_start + 0.6)

                # Format clause into <= 2 lines (max 16 chars/line)
                ...
                for line in lines:
                    if len(line) > 18:
                        violations.append({"shot_id": sid, "clause": clause, "formatted": formatted_text, "error": f"Line length {len(line)} > 18"})
                ...
                clause_t = sub_end + 0.05

            cur_t = clause_t + 0.05

    ass_content = header + "\n".join(dialogue_events) + "\n"
    OUT_ASS.write_text(ass_content, encoding="utf-8")
    print(f"Generated {len(dialogue_events)} 92pt subtitle events into: {OUT_ASS}")

    # Check overlaps
    overlap_count = 0
    for i in range(len(all_intervals) - 1):
        s1, e1, t1 = all_intervals[i]
        s2, e2, t2 = all_intervals[i+1]
        if s2 < e1:
            overlap_count += 1
            violations.append({"overlap": True, "e1": (s1, e1, t1), "e2": (s2, e2, t2)})
    ...
    print(f"QA Status: {qa_data['qa_status']} (Violations: {len(violations)}, Overlaps: {overlap_count})")

    if violations:
        print(f"Found {len(violations)} violations:")
        for v in violations[:5]:
            print(v)
        raise ValueError(f"Found {len(violations)} violations!")
