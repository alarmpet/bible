# -*- coding: utf-8 -*-
"""
pilot_5_rank2_gate_verifier.py
Verifies Pilot-5 (SHOT_001 ~ SHOT_005, 23.5s) for Rank 2 ('잊혀진 문명, 세계 최강이 사라진 이유')
across Gate 3 (Pacing), Gate 4 (Audio Contract & Preflight),
Gate 5 (Asset Contract & Preflight), and Gate 6 (Render Motion Binding).
...
def probe_service(url: str, timeout: float = 1.0) -> bool:
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "PilotGateVerifierRank2/1.0"})
...
def verify_gate3_pacing(bundle: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if abs(opening_dur - 12.0) > 0.001:
        raise ValueError(f"Gate 3 FAIL: Opening duration must be 12.0s, got {opening_dur}s")

    # 2. Tier 1 first 2 cuts timing: 11.5s (5.5s, 6.0s)
    ...
    if abs(tier1_dur - 11.5) > 0.001:
        raise ValueError(f"Gate 3 FAIL: Tier 1 pilot duration must be 11.5s, got {tier1_dur}s")
    ...
    if abs(current_time - 23.5) > 0.001:
        raise ValueError(f"Gate 3 FAIL: Total duration must be 23.5s, got {current_time}s")
    ...
    # 5. Look type diversity: >= 4 distinct look types
...
def verify_gate4_audio_contract(
    ...
    target_dur = bundle.get("target_duration_sec", 23.5)
    parity_diff = abs(total_audio_budget - target_dur)
...
def verify_gate5_asset_contract(
    ...
    """Gate 5 — Asset Gate verification and preflight probe (2D Ligne Claire Webtoon)."""
    shots = bundle.get("shots", [])
    ...
    required_aesthetic_keywords = ["2d ligne claire", "cel-shaded", "webtoon", "ink line art"]
    ...
    for s in shots:
        ...
        # Check 2D Ligne Claire aesthetic markers
        found_aesthetic = [kw for kw in required_aesthetic_keywords if kw in prompt_en]
        if len(found_aesthetic) < 2:
            raise ValueError(f"Gate 5 FAIL: Shot {s['shot_id']} missing 2D Ligne Claire markers: found {found_aesthetic}")

        # Subtitle clearance rule: bottom 18% clear zone
        if "bottom 18%" not in prompt_en:
            raise ValueError(f"Gate 5 FAIL: Shot {s['shot_id']} missing bottom 18% subtitle clear zone instruction")
...
def verify_gate6_render_motion_binding(bundle: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for s in shots:
        ...
        planned_profile = PlannedProfile(
            ...
            sub_type="biphasic_ken_burns" if canonical_motion == "biphasic_ken_burns" else "canonical_subpixel",
...
def run_full_pilot_verification(check_live_services: bool = False, auto_start: bool = True) -> Dict[str, Any]:
    """Execute Gate 3~6 verification pipeline on Pilot-5 for Rank 2."""
    if not PILOT_BUNDLE_FILE.exists():
        raise FileNotFoundError(f"Pilot-5 bundle missing: {PILOT_BUNDLE_FILE}.")

    with open(PILOT_BUNDLE_FILE, "r", encoding="utf-8") as f:
        bundle = json.load(f)

    report = {
        "title": "Human Library Rank 2 Pilot-5 Multi-Gate Preflight Report",
