# -*- coding: utf-8 -*-
"""Generate 58pt Bold Dynamic ASS subtitles with strict <= 2 lines invariant,
recursive clause time slicing for long sentences, and clause-aware syntactic line breaking."""
...
def split_sentence_into_clauses(text: str, max_chars: int = 36) -> list[str]:
    text = text.strip()
    if len(text) <= max_chars:
        return [text]

    # Find punctuation comma near middle (avoiding numbers like 5,000)
    ...
    for pos in comma_matches:
        diff = abs(pos - mid)
        if diff < min_diff and pos <= len(text) - 8 and pos >= 8:
            min_diff = diff
            best_split = pos

    if best_split <= 0:
        # Connective verb endings (~면, ~고, ~며, ~지만, ~는데, ~어서, ~하여, ~되어, ~수록)
        conn_matches = [m.end() for m in re.finditer(r"(?:면|고|며|지만|는데|어서|하여|되어|수록)\s+", text)]
...
def split_clause_two_lines(text: str, max_line_chars: int = 20) -> str:
    text = text.strip()
...
def main():
    ...
    print(f"Loaded {len(shots)} shots for 58pt subtitle compilation.")
...
Style: DocuNarrator_v3,Pretendard,58,&H00FFFFFF,&H000000FF,&H000C0C12,&H90000000,-1,0,0,0,100,100,0,0,1,4.8,2.5,2,140,140,85,1
...
"""
    ...
    for s in shots:
        ...
        for idx, sent in enumerate(raw_sents):
            ...
            sent_dur = max(1.8, round(speech_dur * ratio, 3))
            
            # Subdivide into 1, 2 or 3 clauses if sentence > 36 chars
            clauses = split_sentence_into_clauses(sent, max_chars=36)
            ...
            for c_idx, clause in enumerate(clauses):
                ...
                # Format clause into <= 2 lines
                formatted_text = split_clause_two_lines(clause, max_line_chars=20)
                ...
                for line in lines:
                    if len(line) > 26:
                        violations.append({"shot_id": sid, "clause": clause, "formatted": formatted_text, "error": f"Line length {len(line)} > 26"})

                start_ass = seconds_to_ass(sub_start)
                end_ass = seconds_to_ass(sub_end)
                dialogue_events.append(f"Dialogue: 0,{start_ass},{end_ass},DocuNarrator_v3,,0,0,0,,{formatted_text}")

                clause_t += c_dur

            # Small inter-sentence breath pause
            cur_t = cur_t + sent_dur + 0.15

    ass_content = header + "\n".join(dialogue_events) + "\n"
    OUT_ASS.write_text(ass_content, encoding="utf-8")
    print(f"Generated {len(dialogue_events)} subtitle events into: {OUT_ASS}")

    qa_data = {
        "qa_status": "PASS" if len(violations) == 0 else "FAIL",
        "font_size": 58,
        "style_name": "DocuNarrator_v3",
