# 최종 영상 단일 규칙

모든 생성기는 `bible_healing/config/media_rules_lock.json`을 단일 기준으로 사용한다. `manual.md`, `CLAUDE.md`, `bible_healing/HANDOFF.md`, 연구·계획 문서와 런타임 설정이 다르면 렌더를 중단하고 이 잠금 파일을 기준으로 동기화한다.

## 음성 (2026-08-18 고정)

| | 여성 narrator | 남성 scripture |
|---|---|---|
| 보이스 | **F5** | **M4** |
| 엔진 속도 | **0.95** | **0.86** (10% 감속으로 차분한 낭독) |
| total_step | 8 | **10** (24 금지) |
| 조각 사이 쉼 | 0.24초 | **0.35초** (동일 화자 간격 0.40초) |
| 피치 | **-4%** (`asetrate=24000*0.96`, **atempo 없음**) | **-10%** (`asetrate=24000*0.90`, **atempo 없음**) |
| 체감 속도 | 약 **0.91** (0.95×0.96) | 약 **0.77** (0.86×0.90) |
| 필터 | highpass 60Hz, lowpass 6000Hz, EQ 180+3 / 120+2 (`09_F5_less_thin`) | 동일 EQ (`09_M4_less_thin`) |
| 합성 단위 | 장면 문장 | **절(마침표) 하나 = TTS 1회**, `max_chunk_length` 90, 어절 중간 절단 금지 |

- 화자는 `narrator`와 `scripture`만 허용한다. F3/M2/M3/M5 금지.
- TTS 입력에서 `! ！ !? ❗ ? ？`는 마침표, `<laugh> <breath> <sigh>`·표제·셀라 금지.
- TTS 숫자는 한글 읽기, 자막은 아라비아 숫자 유지. 합성 후 엔진 패딩을 자른다.
- 같은 화자 0.40초(숨 쉴 틈 확보) / 장면·화자 전환 0.6초는 조립 단계. HTTP `:3093` 우선, 꺼지면 in-process SuperTonic. 새 TTS 설치 금지.
- 자막 시간은 필터·트림 **후** 실제 WAV 길이를 쓴다. `--skip-existing` 금지.

## 자막

- 실제 authoritative 음성의 장면별 시작·종료 시각에서 만든다.
- 최대 2줄, 한 줄 목표 14~18자, 최대 20자. 조사·어미·어절 중간 분할 금지.
- **하단 본문 자막**: 본문 96px, 성경 100px, outline 6, shadow 3, marginV 90 (Malgun Gothic).
- **우측 상단 챕터 자막**: 성경 권/장/절(`- 시편 4편 -`) 또는 큰 주제(`- 오프닝 -`)를 **하단 자막 크기의 약 70%인 70px (Alignment 9)**로 상단 우측에 지속 표시.
- 표제·셀라·느낌표가 ASS에 남아 있으면 배포하지 않는다.
- 음성과 자막의 시작·종료 차이는 0.5초 이내여야 한다.

## 영상·배경·저장

- 배경은 `pingpong-1min`의 12개 1분 MP4를 순환 사용하며, **0.1배속 초슬로우 모션 (`setpts=10*PTS`)**으로 재생한다.
- **배경 전환 주기**: 각 배경 영상은 **정확히 1분(60초)마다 다음 샘플로 순환 전환**된다 (각 영상의 0.1배속 60초 구간 순차 렌더링).
- 정지 이미지와 과거 MP4의 오디오 트랙을 최종 입력으로 사용하지 않는다.
- 최종 산출물은 `D:\bible_healing_ep01\final`, 임시 파일은 `D:\bible_healing_ep01\work`에 저장한다.
- `media_rules_preflight.py`가 실패하면 렌더하지 않고, `media_rules_postflight.py`가 통과하지 않으면 배포하지 않는다.
