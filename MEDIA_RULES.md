...
모든 생성기는 `bible_healing/config/media_rules_lock.json`을 단일 기준으로 사용한다. `manual.md`, `CLAUDE.md`, 연구·계획 문서와 런타임 설정이 다르면 렌더를 중단하고 이 잠금 파일을 기준으로 동기화한다.

## 음성

- 화자 종류는 `narrator`와 `scripture` 두 개만 허용한다.
- narrator는 F5, scripture는 승인된 남성 M4만 사용한다.
- scripture는 속도 0.72, pitch -8%, 문장 종결 뒤 0.65초 쉼을 사용한다.
- F3, M3, M5, 이전 MP4 오디오, 임의 voice loop는 최종 음성으로 금지한다.

## 자막

- 실제 authoritative 음성의 장면별 시작·종료 시각에서 만든다.
- 최대 2줄, 한 줄 목표 14~18자, 최대 20자.
- 문장부호·절·어절 경계 우선이며 조사·어미·단어 중간 분할을 금지한다.
