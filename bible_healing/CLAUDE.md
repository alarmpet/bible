...
> 2026-09-15: 원래 저장소 루트 `CLAUDE.md`가 이 문서였으나, `human_archive`(Nollam File) 작업이 같은 경로를 자체 운영 매뉴얼로 대체하면서 두 프로젝트가 루트 파일 하나를 두고 충돌했다. 루트는 `human_archive`가 쓰고, bible_healing 고유 지침은 이 파일(`bible_healing/CLAUDE.md`)로 옮겼다. 경로 표기는 기존과 동일하게 저장소 루트 기준(`bible_healing/...`)을 유지한다.
...
- 우측 상단(Alignment 9)에 현재 읽고 있는 성경 권/장/절(`- 시편 4편 -`, `- 시편 27편 1-5절 -`) 또는 주제(`- 오프닝 -`)를 지속적으로 표시한다.
- 크기는 **하단 본문 자막 크기의 약 70%인 70px (1080p 기준)**로 한다.
- 자막 스타일: `Style: Chapter,Malgun Gothic,70,&H00E8E0D0,&H00E8E0D0,&H00000000,&H90000000,0,0,0,0,100,100,0,0,1,3,2,9,120,120,90,1`
- 영상에서 사라지지 않고 주제/구절이 바뀔 때 문구만 교체한다 (연속된 씬 병합 유지).

## 에피소드 제작 원장 및 중복 방지 규칙 (EPISODE_LEDGER)

- 한 번 완성된 에피소드 영상(`D:\bible_healing_epXX\final\...`)은 절대 덮어쓰거나 삭제하지 않고 로컬에 영구 보존한다.
- 제작 완료 이력은 `EPISODE_LEDGER.md`에 기록하여 관리한다.
- **성경 권(Book) 순환 원칙**: 다음 에피소드 기획 시, 직전 에피소드에서 낭독된 구절과 중복되지 않도록 **시편, 잠언, 전도서, 이사야/선지서, 모세오경** 등 성경 권을 다양하게 순환 배치한다 (중복률 0%).
