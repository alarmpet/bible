...
def opening_rows(bank: dict, override_path: Path | None = None) -> list[dict]:
    """Return opening copy, applying an episode-specific hook override when present."""
    source = bank
...
def expand_unit(pick: dict, bank_unit: dict) -> list[dict]:
    ...
    segs = [
        {"seg_id": f"{uid}_n0", "unit": uid, "speaker": "narrator", "ref": None, "text": clean(bank_unit["before"]), "title": title},
        {"seg_id": f"{uid}_s0", "unit": uid, "speaker": "scripture", "ref": ref, "ref_label": label, "text": scripture, "title": title},
    ]
...
def build(episode_id: str) -> Path:
    ...
    segments = [opening_segment(row, i) for i, row in enumerate(opening_rows(bank, override_path), 1)]
    verses_used = []
