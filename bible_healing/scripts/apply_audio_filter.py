...
def scripture_ffmpeg_filter(lock: dict | None = None, pitch_percent: float = -10.0) -> str:
    ...
    locked_pitch = float((lock.get("voice") or {}).get("scripture", {}).get("pitch", -10))
...
def apply_scripture_filter(src: Path, dst: Path, pitch_percent: float = -10.0) -> dict:
