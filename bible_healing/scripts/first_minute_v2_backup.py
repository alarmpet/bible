...
def _timed_items(manifest: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    cursor = 0.0
    for index, item in enumerate(sorted(rows, key=lambda row: int(row.get("order", index + 1))), 1):
...
def summarize_first_minute(manifest: dict[str, Any], scenes: list[dict[str, Any]]) -> dict[str, Any]:
    """Return first-minute measurements and violations from locked or raw TTS timing."""
    ...
    for item in items:
        ...
        speaker = meta.get("speaker") or scene.get("speaker")
        if speaker == "scripture" and first_scripture_start is None:
    ...
    return {
        ...
        "first_scripture_in_target_window": first_scripture_start is not None and 45.0 <= first_scripture_start <= 55.0,
        "phases": phases,
