# -*- coding: utf-8 -*-
"""
Pack healing script → Hermes-compatible job:
  hermes_jobs/<mode>/scenes.json
  hermes_jobs/<mode>/voice_map.json
  hermes_jobs/<mode>/job.json (minimal)
"""
...
def pack(episode_id: str, mode: str = "full", smoke_units: int | None = None) -> Path:
    ...
    if smoke_units is not None:
        ...
        # include closing only if full
        segs = [s for s in segs if s["unit"] in keep_units]

    # Group into scenes: one scene per segment for longform control
    scenes = []
    ...
    (jdir / "scenes.json").write_text(
        json.dumps(scenes, ensure_ascii=False, indent=2), encoding="utf-8"
    )

    vm = {
        "schema_version": "1.0",
        "engine": voice_cfg.get("engine", "supertonic3"),
        "preview_approved": bool(voice_cfg.get("preview_approved", False)),
        "notes": "bible_healing: narrator=여성 점잖음, scripture=말씀 보이스. 공유 금지.",
        "speakers": {
            "narrator": dict(voice_cfg["speakers"]["narrator"]),
            "scripture": dict(voice_cfg["speakers"]["scripture"]),
        },
        "rules": {
            "narration_default_speaker": "narrator",
            "narrator_must_not_share_voice_with_characters": True,
        },
        "sample_lines": dict(voice_cfg.get("sample_lines") or {}),
    }
    # strip non-tts keys if any
    for sid in list(vm["speakers"].keys()):
        conf = vm["speakers"][sid]
        vm["speakers"][sid] = {
            "label": conf.get("label", sid),
            "voice": conf["voice"],
            "speed": conf.get("speed", 1.0),
            "total_step": conf.get("total_step", 8),
            "silence_duration": conf.get("silence_duration", 0.2),
            "audio_filter": conf.get("audio_filter", ""),
        }

    (jdir / "voice_map.json").write_text(
    ...
    job = {
        ...
        "mode": jdir.name,
        "title": manifest.get("title_ko"),
        "scene_count": len(scenes),
        "translation": "KRV",
        "disclaimer": "healing narration; not medical advice",
    }
    ...
    pack_report = {
        "ok": True,
        "scenes": len(scenes),
        "speakers": list(vm["speakers"].keys()),
        "voices": {k: v["voice"] for k, v in vm["speakers"].items()},
    }
    (jdir / "reports" / "pack_report.json").write_text(
        json.dumps(pack_report, ensure_ascii=False, indent=2), encoding="utf-8"
    )

    print(json.dumps({"ok": True, "job": str(jdir), **pack_report}, ensure_ascii=False))
    return jdir
...
def main() -> None:
    ...
    ap.add_argument("--smoke-units", type=int, default=None, help="pack only opening + N units")
    args = ap.parse_args()
