...
- 성경 본문에서 괄호 설명, 곡 제목, `다윗의 시`, `영장으로 현악에 맞춘 노래`, `셀라/첼라`, 느낌표·물음표를 제거한다.
- `할지어다`, `하리니`, `이로다`, `이니라`, `하시니라` 등 종결 뒤에는 실제 음성 쉼을 넣는다.
- 남성 낭독은 승인된 M4 설정을 사용한다: 속도 0.72, pitch -8%, 낮고 평안한 톤. F3/M2/M5는 사용하지 않는다.
...
- 전체 배포영상에는 `subtitles-timed-ko.ass` 본문 자막을 반드시 번인한다.
