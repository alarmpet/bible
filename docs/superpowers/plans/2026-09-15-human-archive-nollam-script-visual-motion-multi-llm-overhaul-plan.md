...
**추가 완료 (2026-09-15, 커밋 `53809ee`/병합 커밋, 후속 작업 `task_fcf94ecb`에서):** 당시 미뤘던 "Quarantine" 리라이트 자체도 완료됐다. `orchestrate_deep_tri_model_script()`의 Round 1(3개 하드코딩 제안 dict)과 Round 2(하드코딩 교차비평 dict)를 신설 `lib/tri_model_real_debate.py`가 감싸는 **실제** `run_consensus_round.escalate_claim()` 1회 라운드(codex=proposer, grok=adversary)로 교체했다 — 두 실측 참가자의 합의/불일치 자체가 교차비평이 되므로 두 라운드분 조작을 없애면서도 라운드를 두 번 돌리는 비용은 추가되지 않는다. Gemini/agy 미설치로 세 번째 UI 레인("gemini_36")은 가짜 세 번째 모델 호출을 채우는 대신 Round 3의 실제 문장별 팩트체크를 정직하게 가리키도록 했다. `provenance`도 하드코딩 상수 대신 라운드 실제 완주 여부에 따라 `"live_orchestration"`/`"simulated_fixture"`(fail-closed 유지)로 계산된다. Studio GUI를 이번에도 직접 띄워 클릭 검증하지는 못했지만, `static/index.html`의 이벤트 소비 코드를 전문 확인하고 `emit()` 이벤트 타입·순서·`model` 키가 동일하게 유지됨을 모킹 기반 단위테스트 12개 + 성공/실패 양쪽 경로의 end-to-end 수동 호출로 검증했다. `execute_topic_debate()`와 `tri_model_llm_bridge.py`의 "Gemini 3.8/3.7/3.6" 페르소나 프레이밍은 이번 후속작업 범위 밖으로 의도적으로 남겨뒀다.
