...
> **⚠️ 2026-09-15 철회/격리 고지:** 이 계획서는 다른 채널의 특정 실제 영상을 대본 verbatim 복제·컷타이밍 1:1 매칭·**원본 채널 워터마크/엠블럼 복원**까지 지시하고 있어, 저작권·플랫폼 정책 위반 위험이 명백하다. 사용자 확인 결과 원래 취지도 배포가 아니라 내부 학습이었으므로, 관련 산출물은 `human_archive/research/human_library_benchmark_internal_only/`로 격리했고 발행 경로에서 차단하는 코드 가드를 추가했다(`docs/superpowers/plans/2026-09-15-human-archive-nollam-script-visual-motion-multi-llm-overhaul-plan.md` §6). **이 문서의 지시(특히 브랜딩/워터마크 복원 항목)를 문자 그대로 다시 실행하지 않는다.** 구조적 벤치마크 학습 목적으로만 읽는다.
